    ...
    <main className={styles.container}>
      ...
      <Link href="/">
        <div className="flex gap-2 items-center font-bold">
          ...
          <h1 className="text-2xl">BrainSpark</h1>
        </div>
      </Link>
      <div
        className={`flex flex-col mt-2 ${styles.chatHistory}`}
            ...
            <div key={chat.id} className="mb-5 relative">
              <div
                className={`p-5 rounded-md ${
                  chat.sender == "user" ? styles.user : styles.model
                }`}
              >
                <div className={styles.icons}>
                  <Image
                    src={chat.sender == "user" ? "/user.svg" : "/chat.svg"}
                    alt="logo"
                    width={30}
                    height={30}
                  />
                </div>
                <Markdown className="mt-2">{chat.message}</Markdown>
              </div>
            </div>
          ))}
      </div>
      <div
        className={`flex justify-center items-center gap-5 ${styles.inputContainer}`}
      >
        <div className="relative">
          ...
          <Image
            ...
            width={20}
            height={20}
            className="absolute top-3 right-3 cursor-pointer hover:scale-105 transition-all duration-300 ease-in-out"
